create user gs_log identified by ""
  default tablespace APP_LOG
  quota unlimited on APP_LOG_INDEX
  quota unlimited on FEED_FILE;

grant connect, resource to gs_log;
alter session set current_schema = gs_log;
...
create role gs_log_writer;
create role gs_log_reader;

grant SELECT, INSERT ON prematch_feed_file to gs_log_writer;
grant SELECT, INSERT ON inplay_feed_file to gs_log_writer;
GRANT SELECT, INSERT ON data_change_d30 TO gs_log_writer;
GRANT SELECT, INSERT ON data_change_d7 TO gs_log_writer;
GRANT SELECT, INSERT ON data_change_d1 TO gs_log_writer;

grant SELECT ON prematch_feed_file to gs_log_reader;
grant SELECT ON inplay_feed_file to gs_log_reader;
GRANT SELECT ON data_change_d30 TO gs_log_reader;
GRANT SELECT ON data_change_d7 TO gs_log_reader;
GRANT SELECT ON data_change_d1 TO gs_log_reader;
